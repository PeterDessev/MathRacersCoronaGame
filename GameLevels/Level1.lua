...
---------------------------------------
    ...
    car = display.newImageRect(map, "Content/Cars/redCar.png", carx, cary)
...
--[[     local turn1K = 170.66666931152 --2.1*track.height/5
 ...
 --change axis to turn number later
    ...
    function carMovement()
        time = time + (1/60)
        if timerEnable then
            timeString = "Time: " .. string.format("%.2f", tostring(round(time, 2)))
            tutorialText.text = timeString
        end
        ...
        if(car.y >= 342 and car.y <= 345 and car.x > 250)then
            ...
            function victoryStuff()
                if checkAndUpdate( round(time, 2) ) then
                    composer.gotoScene("loggedInScreen") 
                else if setTime ( time ) then
                    composer.gotoScene("loggedInScreen")
                end
            end
        end
...
function scene:show( event )
    ...
    -- Called when the scene is still off screen (but is about to come on screen).
        carSpeed = 0
        car.x, car.y = 282.57189941406, 341.33334350586 
