...
--Properties of turn:
    ...
    local function turn(ammountIn, endpointIn, turnSpeedIn, initialAngleIn, carOrientationIn, triggerPointIn, carTriggerDirectionIn)
        ...
        function self.getTurnAngle(turnName)                                                                            --   Turn Funciton
    ...
    function doTurns(count)
        ...
        if turnCounter <= count then
            ...
            _G[turnNameString].getTurnAngle("turn1")
...
return scene
    ...
    --returnAngle = 0, ammount = 0, endpoint = 0, turnSpeed = 0, initialAngle = 0, carOrientation = "", triggerPoint = 0, carTriggerDirection = ""
--[[local turn = {} 
    turn.__index = turn
    ...
    --Creates turn object
    function turn:new(ammount, endpoint, turnSpeed, initialAngle, carOrientation, triggerPoint, carTriggerDirection)
        ...
        self.carTriggerDirection = carTriggerDirection
        return self
    end
    
    function turn:returnAngleV()
        car.rotation = (self.initialAngle + self.turnSpeed * (self.ammount*((car.y - self.triggerPoint)/(self.endpoint - self.triggerPoint))))
    end 
    
    function turn:returnAngleH()
        car.rotation = (self.initialAngle + self.turnSpeed * (self.ammount*((car.y - self.triggerPoint)/(self.endpoint - self.triggerPoint))))
    end 
        
    --turn:new(nil, -90, 160, -2.2, 360, "v", 170.66666931152, "up")
    --Checks for when the car needs to turn accounting for which dirrection the car is going to turn from, returns true when turn is complete; can still be used for irregular turn angles
    --turn2 = turn:new(nil, -90, 50, 1, 270, "h", 100, "left")
    function turn:getTurnAngle(turnName)                                                                            --   Turn Funciton
        if self.carOrientation == "v" then                                                                          --   |-- Car is turning from virticle position
            if self.carTriggerDirection == "up" then                                                                --   |   |-- Car is facing up 
                if car.y <= self.triggerPoint then                                                                  --   |   |   |-- Car is above trigger
                    if car.y <= (self.endpoint + 15) and round(car.rotation, 0) == (self.initialAngle + self.ammount) then     --   |   |   |   |-- Car is below endpoint
                        return true                                                                                 --   |   |   |   |   |-- Returns true to mark turn complete
                                                                                                                    --   |   |   |   |
                    else                                                                                            --   |   |   |   |-- Car is above endpoint
                        self:returnAngleV()                                                                 --   |   |   |       |-- Car starts to turn
                    end                                                                                             --   |   |   | 
                end                                                                                                 --   |   |   |-- Car is below  trigger, turn doesn't start yet
                                                                                                                    --   |   |
            elseif self.carTriggerDirection == "down" then                                                          --   |   |-- Car is facing down
                if car.y >= self.triggerPoint then                                                                  --   |   |   |-- Car is below trigger
                    if car.y <= (self.endpoint - 15) and round(car.rotation, 0) == (self.initialAngle + self.ammount) then    -- |   |-- Car is above endpoint
                        return true                                                                                 --   |   |   |   |   |-- Returns true to mark turn complete
                                                                                                                    --   |   |   |   |
                    else                                                                                            --   |   |   |   |-- Car is below endpoint
                        _G[turnName]:returnAngleV()                                                                 --   |   |   |       |-- Car starts to turn
                    end                                                                                             --   |   |   | 
                end                                                                                                 --   |   |   |-- Car is above trigger, turn doesn't start yet
            end                                                                                                     --   |   |
                                                                                                                    --   |
        elseif self.carOrientation == "h" then                                                                      --   |-- Car is turning from horizontal position
            if self.carTriggerDirection == "right" then                                                             --   |   |-- Car is facing right 
                if car.x >= self.triggerPoint then                                                                  --   |   |   |-- Car is right of the trigger
                    if car.x >= (self.endpoint - 5) and round(car.rotation, 0) == (self.initialAngle + self.ammount) then     -- |   |-- Car is right of endpoint
                        return true                                                                                 --   |   |   |   |   |-- Returns true to mark turn
                                                                                                                    --   |   |   |   |
                    else                                                                                            --   |   |   |   |-- Car is left of endpoint
                        _G[turnName]:returnAngleH()                                                                 --   |   |   |       |-- Car starts to turn complete
                    end                                                                                             --   |   |   | 
                end                                                                                                 --   |   |   |-- Car is left of trigger, turn doesn't start yet
                                                                                                                    --   |   |
            elseif self.carTriggerDirection == "left" then                                                          --   |   |-- Car is facing left
                if car.x <= self.triggerPoint then                                                                  --   |   |   |-- Car is left of the trigger
                    if car.x <= (self.endpoint + 5) and round(car.rotation, 0) == (self.initialAngle + self.ammount) then     -- |   |-- Car is right endpoint
                        return true                                                                                 --   |   |   |   |   |-- Returns true to mark turn complete
                                                                                                                    --   |   |   |   |   
                    else                                                                                            --   |   |   |   |-- Car is left endpoint
                        _G[turnName]:returnAngleH()                                                                 --   |   |   |   |   |-- Car starts to turn
                    end                                                                                             --   |   |   |       
                end                                                                                                 --   |   |   |-- Car is right of trigger, turn doesn't start yet
            end                                                                                                     --   |   |   |
        end                                                                                                         --   |   |
...
--Properties of turn:
    ...
    --function turn:new (o, ammount, endpoint, turnSpeed, initialAngle, carOrientation, triggerPoint, carTriggerDirection)     
    
    turn1 = turn:new(nil, -90, 120, 1, 360, "v", 155, "up")
    turn2 = turn:new(nil, -90, 83, 1, 270, "h", 100, "left")
    turn3 = turn:new(nil, -90, 120, 1, 360, "v", 155, "up")
    
    local w = 1
    function turnOnTrack()
        local turnName = "turn" .. tostring(w)
        if(_G[turnName]:getTurnAngle("turn1") ~= true) then
           _G[turnName]:getTurnAngle("turn1")
            print(_G[turnName]:getTurnAngle("turn1"))
            print(turnName)
        else
            w = w + 1
        end
