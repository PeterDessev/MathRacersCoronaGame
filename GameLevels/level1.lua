...
---------------------------------------
    ...
    print(track)
    --]]
    local mapy = 406.34921264648/1.2
    local mapx = 285.05096435547/1.2
    local track = display.newImageRect(map, "Content/Levels/Level1.png", mapx, mapy)
    track.x, track.y = 160, 264.76190185547  --display.contentWidth/2, (display.contentWidth/1.05)

    local cary = 56.888891601562/1.2 --display.contentHeight/10
    local carx = 35.545639382845/1.2 --(448 * cary)/717
    car = display.newImageRect(map, "Content/Cars/" .. getCar(getCurrentUser()) .. ".png", carx, cary)
    car.x, car.y = 263, 341.3333435058 --display.contentWidth/2 + track.width*0.43, (3*display.contentHeight/5) 
...
--Properties of turn:
    ...
    turn1 = turn(-90, 150-40, 1, 360, "v", 185-40, "up")
    turn2 = turn(-90, 125, 1, 270, "h", 90, "left")
    turn3 = turn(-90, 390-40, 1, 180, "v", 425-40, "down")
    turn4 = turn(-90, 263, 1, 90, "h", 240, "right")    
    local turnCounter = 1
    ...
    function doTurns(count)
        ...
        elseif winCondition("y", "up", 300) then
            if gameWon == false then
                ...
                if checkAndUpdate(milis, "Level 1") then
