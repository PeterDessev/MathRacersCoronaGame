    ...
    local answerButton1 = widget.newButton(
        {
            ...
            font = Arial, 
            fontSize = 20,
            width = (display.contentWidth/4-20),
            height = (40),
...
---------------------------------------
    ...
    --[[
        local mapy = display.contentHeight/1.4
    local mapx = (423 * mapy)/603
    local track = display.newImageRect(map, "Content/Levels/roundTrack.png", mapx, mapy)
    track.x, track.y = display.contentWidth/2, (display.contentWidth/1.05)
    print(track)
    --]]
    local mapy = 406.34921264648
    local mapx = 285.05096435547
    local track = display.newImageRect(map, "Content/Levels/Level2.png", mapx, mapy)
    track.x, track.y = 160, 304.76190185547  --display.contentWidth/2, (display.contentWidth/1.05)

    local cary = 56.888891601562 --display.contentHeight/10
    local carx = 35.545639382845 --(448 * cary)/717
    car = display.newImageRect(map, "Content/Cars/" .. getCar(getCurrentUser()) .. ".png", carx, cary)
    car.rotation = 90
    car.x, car.y = 0, 480--display.contentWidth/2 + track.width*0.43, (3*display.contentHeight/5)     
...
--Properties of turn:
    ...
    local function turn(ammountIn, endpointIn, turnSpeedIn, initialAngleIn, carOrientationIn, triggerPointIn, carTriggerDirectionIn)
        ...
        function self.returnAngleV()
            car.rotation = (initialAngle + ammount * math.abs((car.y - triggerPoint)/(endpoint - triggerPoint)))
        end 
    
        function self.returnAngleH()
            car.rotation = (initialAngle + ammount * math.abs((car.x - triggerPoint)/(endpoint - triggerPoint)))
        end 
        ...
        function self.getTurnAngle()                                                                            --   Turn Funciton 
               if carOrientation == "v" then                                                                          --   |-- Car is turning from virticle position
                   if carTriggerDirection == "up" then                                                                --   |   |-- Car is facing up 
                       if car.y <= (triggerPoint - 5) then                                                                  --   |   |   |-- Car is above trigger
                           if car.y <= (endpoint + 5) and round(car.rotation, 0) == (initialAngle + ammount) then    --   |   |   |   |-- Car is below endpoint
                   ...
                   elseif carTriggerDirection == "down" then                                                          --   |   |-- Car is facing down
                       if car.y >= (triggerPoint + 5) then                                                                  --   |   |   |-- Car is below trigger
                           if car.y >= (endpoint - 5) and round(car.rotation, 0) == (initialAngle + ammount) then    -- |   |-- Car is above endpoint
               ...
               elseif carOrientation == "h" then                                                                      --   |-- Car is turning from horizontal position
                   if carTriggerDirection == "right" then                                                             --   |   |-- Car is facing right 
                       if car.x >= (triggerPoint + 5) then                                                                  --   |   |   |-- Car is right of the trigger
                           if car.x >= (endpoint - 5) and round(car.rotation, 0) == (initialAngle + ammount) then     -- |   |-- Car is right of endpoint
                   ...
                   elseif carTriggerDirection == "left" then                                                          --   |   |-- Car is facing left
                       if car.x <= (triggerPoint - 5) then                                                                  --   |   |   |-- Car is left of the trigger
                           if car.x <= (endpoint + 5) and round(car.rotation, 0) == (initialAngle + ammount) then     -- |   |-- Car is right endpoint
                               return true                                                                                 --   |   |   |   |   |-- Returns true to mark turn complete
                                                                                                                           --   |   |   |   |   
...
--Properties of turn:
    --1: ammount (the angle of the turn, usually 90) 
    --2: endpoint (where on the coordinate system the car needs to stop while turning)
    --3: turnSpeed (equivelent to how sharp the turn is)
    --4: initialAngle (orientation of car before turn)
    --5: carOrientation (What way the car is facing when starting the turn; v irticle v, horizontal h) prolly remove me soon
    --6: triggerPoint (when to turn on coordinate system)
    --7: carTriggerDirection (what side of the trigger point the turn happens (e.g. if the car is going up to a turn or down))
    
    
    turn1 = turn(-90, 50, 1, 90, "h", 30, "right")
        
    turn2 = turn(90, 130, 1, 0, "v", 147, "up")
    turn3 = turn(90, 105, 1, 90, "h", 95, "right")  
        
    turn4 = turn(-90, 467, 1, 180, "v", 455, "down")
    turn5 = turn(-90, 160, 1, 90, "h", 135, "right")  
        
    turn6 = turn(90, 130, 1, 0, "v", 147, "up")  
    turn7 = turn(90, 215, 1, 90, "h", 205, "right")
        
    turn8 = turn(-90, 467, 1, 180, "v", 455, "down")  
    turn9 = turn(-90, 270, 1, 90, "h", 240, "right")
        
    turn10 = turn(90, 130, 1, 0, "v", 147, "up")
    local turnCounter = 1
    ...
    function doTurns(count)
        ...
        elseif winCondition("x", "right", 350) then
            ...
            if gameWon == false then
                ...
                if checkAndUpdate(milis, "Level 2") then
